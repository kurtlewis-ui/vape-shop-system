...
interface ExportSaleItem {
  ...
  note: string | null;
  paymentSplit: { cash: number; gcash: number; bankTransfer: number; cashless: number } | null;
}
...
interface ExportProduct {
  ...
  quantities: { branchName: string | null; quantity: number }[];
}
...
export async function exportAllData(): Promise<void> {
  ...
  const sales: ExportSale[] = salesRes.data.data ?? [];
  const disposals: ExportDisposal[] = disposalsRes.data.data ?? [];
  const expenses: ExportExpense[] = expensesRes.data.data ?? [];
  const products: ExportProduct[] = productsRes.data.data ?? [];
  ...
  const inventoryRows = products.map((p) => [
    ...
    p.quantityAlert,
    ...branchList.map((b) => {
      const q = p.quantities.find((x) => x.branchName === b);
      return q?.quantity ?? 0;
    }),
  ]);
